 ...
 */
void _GET(int file, char *bufferr, unsigned long int c)
{
	if (read(file, bufferr, c) > -1)
		return;
	_putstr("Error: Can't read from file\n");
	_EXIT(file);
	exit(98);
}
...
int main(int argc, const char **argv)
{
	...
	if (argc != 2)
	{
		_putstr("Usage: elf_header elf_filename\n");
		exit(98);
	}
	...
	_GET(fd, (char *) buffer, bit_mode / 8);
